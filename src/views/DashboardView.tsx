...
import { Upload, FileSpreadsheet, Trophy, Clock, MapPin, MousePointerClick, Activity } from 'lucide-react';
...
export const DashboardView: React.FC = () => {
    ...
    // --- 1. HOOKS (USEMEMO) DEVEM FICAR SEMPRE NO TOPO ---
    // (Correção: Movidos para antes do 'if')

    const dadosBarraGoogle = useMemo(() => {
        const header = [["Localidade", "Total Atendimentos"]];
        const body = dadosGraficoBarras.sort((a: any, b: any) => b.total - a.total).slice(0, 15).map((d: any) => [d.nome, d.total]);
    ...
    const dadosLinhaGoogle = useMemo(() => {
        ...
        const body = dadosGraficoLinha.map((d: any) => {
            ...
            if (d.data.includes('-')) labelData = d.data.split('-').slice(1).reverse().join('/');
    ...
    // --- 2. LOGICA DE ROTEAMENTO (SÓ AGORA O RETURN) ---

    // Se tiver um produtor selecionado, renderiza a tela de detalhes
    if (produtorSelecionadoNome) {
        return (
            <ProdutorDetailView
                nome={produtorSelecionadoNome}
                dados={dadosProdutorSelecionado}
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans p-8 transition-all">
            {/* Header */}
            ...
            {!temDados && !isLoading && (
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-20 text-center bg-white">
                    ...
                    <p className="text-gray-500">Faça o upload do CSV.</p>
                </div>
            )}
            ...
            {temDados && !isLoading && (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div
                            onClick={() => top1Produtor && selecionarProdutor(top1Produtor.nome)}
                            className="bg-white p-6 rounded-xl shadow border-l-4 border-yellow-400 flex flex-col justify-between cursor-pointer hover:bg-yellow-50 transition"
                        >
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-sm font-semibold text-gray-500 uppercase">Produtor Destaque (Clique)</p>
                            ...
                            <div className="mt-4 flex items-center gap-2 text-yellow-700 bg-yellow-100 px-3 py-1 rounded-lg w-fit">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow border-l-4 border-green-400 flex flex-col justify-between">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-sm font-semibold text-gray-500 uppercase">Cobertura Regional</p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <h2 className="text-xl font-bold mb-4 text-gray-800">Demandas por Localidade</h2>
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            <h2 className="text-xl font-bold mb-4 text-gray-800">Financeiro</h2>
                            <Chart chartType="BubbleChart" width="100%" height="300px" data={dadosBubbleGoogle} options={{ colorAxis: { colors: ['#8884d8'] }, hAxis: { title: "Estoque" }, vAxis: { title: "Custo" }, bubble: { textStyle: { fontSize: 10 } } }} />
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2">
                            ...
                            Ranking: Melhores Produtores por Região
                        </h2>
                            ...
                            <table className="w-full text-sm text-left text-gray-500">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3">Principal Produtor (Clique para Detalhes)</th>
                                ...
                                <tbody>
                                    {destaquePorRegiao.slice(0, 10).map((d, idx) => (
                                        <tr
                                            key={idx}
                                            onClick={() => selecionarProdutor(d.campeao)}
                                            className="bg-white border-b hover:bg-blue-50 cursor-pointer transition-colors group"
                                        >
                                            <td className="px-6 py-4 font-medium text-gray-900">{d.regiao}</td>
                                            <td className="px-6 py-4 font-bold text-blue-600 group-hover:underline">{d.campeao}</td>
                                            <td className="px-6 py-4 text-right">{d.visitas}</td>
                                            <td className="px-6 py-4 text-right text-gray-400 group-hover:text-blue-500">
                                                <MousePointerClick size={16} className="inline" />
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
